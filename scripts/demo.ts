...
for (const demo of demoScenarios) {
  ...
  console.log(JSON.stringify({ name: demo.name, request: demo.request, status: result.status, partial: result.partial, candidates: result.recommendations.map(r => ({ id: r.contractor.id, name: r.contractor.anon_name, priceKzt: r.contractor.price_from_kzt, decisionMeta: r.decisionMeta })), decision: result.decision, funnel: result.funnel, reasons: result.rejectionReasons, counterfactuals: result.counterfactuals }, null, 2));
}
